...
object BuilderValidator {
    ...
    fun validateNoDuplicateAliasInNewConceptAnnotation(method: KFunction<*>, aliasesFromSuperiorBuilder: Set<Alias>) {
        ...
        if(duplicateAlias != null) {
            ...
            val concept = allNewConceptAnnotations
                ...
                .last()
            throw BuilderMethodSyntaxException(method, BuilderErrorCode.ALIAS_IS_ALREADY_USED, duplicateAlias, concept,  allUsedAliases, defaultAliasHint(duplicateAlias))
        }
    }
    ...
    fun validateConceptIdentifierAssignment(method: KFunction<*>, aliasesFromNewConceptAssignment: Set<Alias>) {
        ...
        if(duplicateAssignmentAlias != null) {
            throw BuilderMethodSyntaxException(method, BuilderErrorCode.DUPLICATE_SET_CONCEPT_IDENTIFIER_VALUE_USAGE, duplicateAssignmentAlias, defaultAliasHint(duplicateAssignmentAlias))
        }
        ...
        firstMissingAlias(setRandomConceptIdentifierAliases, aliasesFromNewConceptAssignment)?.let { unknownAlias ->
            throw BuilderMethodSyntaxException(method, BuilderErrorCode.UNKNOWN_ALIAS, SetRandomConceptIdentifierValue::class.annotationText(), unknownAlias, aliasesFromNewConceptAssignment, defaultAliasHint(unknownAlias))
        }
        ...
        firstMissingAlias(setConceptIdentifierValueAliases, aliasesFromNewConceptAssignment)?.let { unknownAlias ->
            throw BuilderMethodSyntaxException(method, BuilderErrorCode.UNKNOWN_ALIAS, SetConceptIdentifierValue::class.annotationText(), unknownAlias, aliasesFromNewConceptAssignment, defaultAliasHint(unknownAlias))
        }
    }
    ...
    private fun validateIsValidAlias(
        ...
        if(alias !in knownConceptAliases) {
            if(methodParameter == null) {
                throw BuilderMethodSyntaxException(method, BuilderErrorCode.UNKNOWN_ALIAS, alias.name, annotation.annotationClass.annotationText(), knownConceptAliases, defaultAliasHint(alias))
            } else {
                throw BuilderMethodParameterSyntaxException(method, methodParameter, BuilderErrorCode.UNKNOWN_ALIAS, alias.name, annotation.annotationClass.annotationText(), knownConceptAliases, defaultAliasHint(alias))
            }
        }
    }
    ...
    private fun validateIsValidFacet(
        ...
        if(!concept.hasFacet(facet)) {
            if(methodParameter == null) {
                throw BuilderMethodSyntaxException(method, BuilderErrorCode.UNKNOWN_FACET, annotation.annotationClass.annotationText(), facet)
            } else {
                throw BuilderMethodParameterSyntaxException(method, methodParameter, BuilderErrorCode.UNKNOWN_FACET, annotation.annotationClass.annotationText(), facet)
            }
    ...
    fun validateKnownConceptsFromNewConceptAnnotation(method: KFunction<*>, schemaAccess: SchemaAccess) {
        method.annotations.filterIsInstance<NewConcept>().forEach { newConceptAnnotation ->
            ...
            if(!schemaAccess.hasConceptName(conceptName)) {
                throw BuilderMethodSyntaxException(method, BuilderErrorCode.UNKNOWN_CONCEPT, conceptAlias, conceptName, defaultAliasHint(conceptAlias))
            }
        }
    }
