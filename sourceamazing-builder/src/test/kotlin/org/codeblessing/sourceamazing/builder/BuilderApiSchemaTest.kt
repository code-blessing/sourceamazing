...
class BuilderApiSchemaTest {
    ...
    fun `test using NewConcept annotation with known concept should not fail`() {
        SchemaApi.withSchema(schemaDefinitionClass = SchemaWithConceptWithFacet::class) { schemaContext ->
            BuilderApi.withBuilder(schemaContext, BuilderMethodCreatingKnownConcept::class) { builder ->
                // do nothing
            }
        }
    }
    ...
    @Test
    fun `test using NewConcept annotation with unknown concept should throw an error`() {
        assertBuilderMethodSyntaxException(BuilderErrorCode.UNKNOWN_CONCEPT) {
            SchemaApi.withSchema(schemaDefinitionClass = SchemaWithConceptWithFacet::class) { schemaContext ->
                BuilderApi.withBuilder(schemaContext, BuilderMethodCreatingUnknownConcept::class) { builder ->
                    // do nothing
                }
            }
        }
    ...
    @Test
    fun `test using unknown facet as fixed value of known concept should throw an error`() {
        assertBuilderMethodSyntaxException(BuilderErrorCode.UNKNOWN_FACET) {
            SchemaApi.withSchema(schemaDefinitionClass = SchemaWithConceptWithFacet::class) { schemaContext ->
                BuilderApi.withBuilder(schemaContext, BuilderMethodUsingUnknownFacetAsFixedValue::class) { builder ->
                    // do nothing
                }
            }
        }
    ...
    fun `test using known facet of another concept as fixed value should throw an error`() {
        assertBuilderMethodSyntaxException(BuilderErrorCode.UNKNOWN_FACET) {
            SchemaApi.withSchema(schemaDefinitionClass = SchemaWithConceptWithFacet::class) { schemaContext ->
                BuilderApi.withBuilder(schemaContext, BuilderMethodUsingFacetOfAnotherKnownConceptAsFixedValue::class) { builder ->
                    // do nothing
                }
            }
        }
    ...
    @Test
    fun `test using known facet of correct concept as parameter value in nested builder should throw an error`() {
        SchemaApi.withSchema(schemaDefinitionClass = SchemaWithConceptWithFacet::class) { schemaContext ->
            BuilderApi.withBuilder(schemaContext, BuilderMethodPassingCorrectConceptParentNestedBuilderAsParameterValue::class) { builder ->
                // do nothing
            }
        }
    }
    ...
    @Test
    fun `test using known facet of correct concept as fixed value in nested builder should throw an error`() {
        SchemaApi.withSchema(schemaDefinitionClass = SchemaWithConceptWithFacet::class) { schemaContext ->
            BuilderApi.withBuilder(schemaContext, BuilderMethodPassingCorrectConceptParentNestedBuilderAsFixedValue::class) { builder ->
                // do nothing
            }
        }
    }
    ...
    fun `test using known facet of wrong concept as fixed value in nested builder should throw an error`() {
        assertBuilderMethodSyntaxException(BuilderErrorCode.UNKNOWN_FACET) {
            SchemaApi.withSchema(schemaDefinitionClass = SchemaWithConceptWithFacet::class) { schemaContext ->
                BuilderApi.withBuilder(schemaContext, BuilderMethodPassingWrongConceptParentNestedBuilderAsFixedValue::class) { builder ->
                    // do nothing
                }
            }
        }
