...
object KTypeUtil {
    private val KOTLIN_PRIMITIVE_ARRAY_TYPES =
        setOf(
            BooleanArray::class,
            IntArray::class,
            LongArray::class,
    ...
    data class KTypeClassInformation(val kind: KTypeKind, val clazz: KClass<*>, val isValueNullable: Boolean)

    fun classInformationFromClass(kClass: KClass<*>, isNullable: Boolean): KTypeClassInformation {
        return KTypeClassInformation(KTypeKind.KCLASS, kClass, isNullable)
    }
    ...
    fun classesInformationFromKType(kType: KType): List<KTypeClassInformation> {
        ...
        classInfos.add(KTypeClassInformation(kType.typeKind(), classifier, kType.isMarkedNullable))
        kType.arguments.forEach { kTypeProjection ->
            val argumentKType = kTypeFromProjection(kTypeProjection)
            classInfos.add(
                KTypeClassInformation(
                    argumentKType.typeKind(),
                    classFromType(argumentKType),
                    argumentKType.isMarkedNullable,
                )
            )
        }
        return classInfos
    ...
    fun kTypeFromProjection(
        ...
        val type = requireNotNull(projection.type) { "type must have a declared function type." }
    ...
    fun classFromType(type: KType): KClass<*> {
        ...
        requireNotNull(classifier) { "type classifier was null." }
        require(classifier is KClass<*>) { "type classifier is not a class but was $classifier." }
