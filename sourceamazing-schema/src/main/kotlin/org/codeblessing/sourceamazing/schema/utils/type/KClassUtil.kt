...
object KClassUtil {
    ...
    fun isAbstract(classToInspect: KClass<*>): Boolean {
        return classToInspect.isAbstract
    }

    fun isEnum(classToInspect: KClass<*>): Boolean {
        return classToInspect.isEnum
    }

    fun isPrivate(classToInspect: KClass<*>): Boolean {
        return classToInspect.isPrivate
    }
    ...
    fun findAllCommonBaseClasses(classes: List<KClass<*>>): Set<KClass<*>> {
        ...
        classes.forEach { clazz ->
            ...
            thisClassAndItsSuperclasses.forEach { classInHierarchy ->
                val isCommonBaseClass = otherClasses.all { otherClazz -> otherClazz.isSubclassOf(classInHierarchy) }
