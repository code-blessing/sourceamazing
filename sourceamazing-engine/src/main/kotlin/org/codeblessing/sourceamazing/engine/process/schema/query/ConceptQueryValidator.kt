    ...
    fun validateAccessorMethodsOfConceptClass(conceptClass: KClass<*>) {
        ...
        conceptClass.java.methods.forEach { method ->
            if(method.parameterCount > 0) {
                throw MalformedSchemaException("The following method has arguments/parameters " +
                        "which is not allowed for methods annotated with " +
                        "${QueryConcepts::class.shortText()} or ${QueryConceptId::class.shortText()}: $method")
            }
            ...
            if(AnnotationUtil.hasAnnotation(method, QueryFacet::class)) {
                ...
                throw MalformedSchemaException("The following method is missing " +
                        ...
                        "or ${QueryConceptId::class.shortText()}: $method")
            }
        }
    }
}
