    ...
    fun formContent(form: FormSchema.FormConcept): String {
        ...
        form.formControls.forEach { formControl ->
            if(formControl is FormSchema.TextInputFormControlConcept) {
                content += """    <label>${formControl.displayName}${if(formControl.valueRequired) "*" else ""}</label>""" + "\n"
                ...
                content += """    <label>${formControl.displayName}${if(formControl.valueRequired) "*" else ""}</label>""" + "\n"
                content += """    <select name="${formControl.displayName}" option="${formControl.defaultValue ?: ""}">""" + "\n"
                formControl.selectDropdownEntries
                    .forEach { optionEntry -> content += """      <option value="${optionEntry.value}">${optionEntry.displayValue}</option>""" + "\n" }
                content += """    </select>""" + "\n"
            }
        }
        content += """  </form>""" + "\n"
    ...
    fun formsSummary(forms: List<FormSchema.FormConcept>): String {
        ...
        forms.forEach { entity ->
            ...
            entity.formControls.forEach { formControl ->
                val formControlSummary = "Form-Control: Display Name: '${formControl.displayName}', Labels: [${formControl.labels.joinToString()}]"
                ...
                if(formControl is FormSchema.TextInputFormControlConcept) {
                    ...
                    val options = formControl.selectDropdownEntries.joinToString { optionEntry -> "[${optionEntry.value} -> '${optionEntry.displayValue}']" }
                    content += """
            ...
            val listOfTextInputFormControlNames = entity.formControls.filterIsInstance<FormSchema.TextInputFormControlConcept>().joinToString { textInputFormControl -> textInputFormControl.displayName }
            content += """
